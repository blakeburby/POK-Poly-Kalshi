...
apply_deploy_env_policy() {
  ...
  # Auto-hardlocks/circuit-breakers AND the execution-quality gate are OFF by default (operator choice
  # 2026-06-24) and the policy intentionally does NOT force these keys, so an explicit
  # LIVE_AUTO_HARDLOCKS_ENABLED=true / LIVE_EXECUTION_QUALITY_GATE_ENABLED=true in worker.env persists across
  # deploys. To re-enable the loss-cap/underfill/fill-mismatch backstops or the quality gate, set the env var(s).
