...
const money = (r: Rec | null, centsKeys: string[], dollarKeys: string[] = []): number | null => {
  ...
  if (d != null) return round(d);
  const c = firstNum(r, centsKeys);
  return c == null ? null : round(c / 100);
};
...
const kalshiSettledMs = (s: Rec): number | null => {
  const t = Date.parse(String(s.settled_time ?? s.settled_at ?? ""));
  return Number.isFinite(t) ? t : null;
};
// Polymarket window start is encoded in the slug, e.g. "btc-updown-15m-1781714700".
const polyWindowMs = (p: Rec): number | null => {
  const m = /(\d{9,})/.exec(String(p.slug ?? p.eventSlug ?? ""));
...
async function main(): Promise<void> {
  ...
  out.cutoff = cutoffs
    ? { input: process.argv[2], asUTC: new Date(cutoffs.utc).toISOString(), asET: new Date(cutoffs.et).toISOString() }
    : "none (lifetime)";
  ...
  try {
    ...
    const settCost = (s: Rec) =>
      (firstNum(s, ["yes_total_cost_dollars"]) ?? 0) + (firstNum(s, ["no_total_cost_dollars"]) ?? 0);
    ...
    const fillCash = sum(
      btcFills.map((f) => {
        const isSell =
          (firstStr(f, ["action", "side"]) ?? "").toLowerCase().includes("sell") ||
          (firstStr(f, ["book_side"]) ?? "") === "ask";
    ...
    const openExposure = sum(
      btcPos
        .filter((p) => Math.abs(firstNum(p, ["position", "position_fp", "net_position"]) ?? 0) > 1e-6)
    ...
    const netSince = (cutoffMs: number) => {
      const inWin = btcSettled.filter((s) => {
        const t = kalshiSettledMs(s);
        return t != null && t >= cutoffMs;
      });
      return {
        count: inWin.length,
        realizedNet: sum(inWin.map((s) => round(settRevenue(s) - settCost(s) - settFee(s)))),
        fees: sum(inWin.map(settFee)),
      };
    };
    const settledTimes = btcSettled
      .map(kalshiSettledMs)
    ...
    out.kalshi = {
      counts: {
        ...
        btcPositions: btcPos.length,
        btcSettlements: btcSettled.length,
        btcFills: btcFills.length,
        fillsTruncated: btcFills.length < btcSettled.length,
      },
      settledTimeRange: settledTimes.length
        ? {
            earliest: new Date(settledTimes[0]).toISOString(),
            latest: new Date(settledTimes[settledTimes.length - 1]).toISOString(),
          }
        : null,
    ...
    out.kalshi = { ERROR: e instanceof Error ? e.message : String(e) };
  }

  // ---- POLYMARKET (BTC 15m) ----
  try {
    const address =
      config.polymarketFunderAddress?.trim() ||
      privateKeyToAccount(
        (config.polymarketPrivateKey.trim().startsWith("0x")
          ? config.polymarketPrivateKey.trim()
          : `0x${config.polymarketPrivateKey.trim()}`) as `0x${string}`,
      ).address;
    ...
    const openExposure = sum(
      btc
        .filter((p) => !isEnded(p) && (firstNum(p, ["size"]) ?? 0) > 1e-6)
    ...
    const polyNetSince = (cutoffMs: number) => {
      const inWin = btc.filter(isEnded).filter((p) => {
        const t = polyWindowMs(p);
        return t != null && t >= cutoffMs;
      });
    ...
    out.polymarket = {
      address,
      btcPositions: btc.length,
      totalPositions: positions.length,
      endedCount: btc.filter(isEnded).length,
      unredeemedCount: unredeemed.length,
      windowRange: polyTimes.length
        ? {
            earliest: new Date(polyTimes[0]).toISOString(),
            latest: new Date(polyTimes[polyTimes.length - 1]).toISOString(),
          }
        : null,
    ...
    out.polymarket = { ERROR: e instanceof Error ? e.message : String(e) };
  }
  ...
  out.COMBINED = {
    ...
    lifetime_realizedTakeHome_netFees: round(
      n(kLife?.realized_via_settlements_net) + n(pLife?.realizedTakeHome_netFees),
    ),
    newModel_sinceCutoff_UTC: cutoffs
      ? {
          realizedTakeHome_netFees: round(n(kUtc?.realizedNet) + n(pUtc?.realized)),
          kalshi: round(n(kUtc?.realizedNet)),
          kalshiFees: round(n(kUtc?.fees)),
          polymarket: round(n(pUtc?.realized)),
          kalshiSettlements: kUtc?.count ?? 0,
          polyPositions: pUtc?.count ?? 0,
        }
      : null,
    newModel_sinceCutoff_ET: cutoffs
      ? {
          realizedTakeHome_netFees: round(n(kEt?.realizedNet) + n(pEt?.realized)),
          kalshi: round(n(kEt?.realizedNet)),
          kalshiFees: round(n(kEt?.fees)),
          polymarket: round(n(pEt?.realized)),
          kalshiSettlements: kEt?.count ?? 0,
          polyPositions: pEt?.count ?? 0,
        }
      : null,
