...
import {
  buildKalshiSubscribeMessage,
  KalshiOrderbookParser,
  KalshiTickerClient,
  parseKalshiTickerSnapshot,
...
test("websocket rate-limit and reconnect delays follow backoff policy", () => {
  ...
  assert.deepEqual(computeReconnectDelay({ attempt: 1, now: 1_000, rateLimitBackoffUntil: 31_000 }), {
    delayMs: 30_000,
    reason: "rate_limited",
  });
});
...
test("Polymarket CLOB websocket parser ignores heartbeat text frames", () => {
  ...
  assert.deepEqual(
    parsePolymarketBookSocketPayload(Buffer.from('{"event_type":"best_bid_ask","asset_id":"token","best_ask":"0.45"}')),
    {
      event_type: "best_bid_ask",
      asset_id: "token",
      best_ask: "0.45",
...
test("Polymarket user websocket parser ignores benign text control frames", () => {
  ...
  assert.throws(() => parsePolymarketUserStreamSocketPayload(Buffer.from("INVALID OPERATION")), /Unexpected token/);
});
...
test("shouldForceFeedReconnect fires only on an open, subscribed, silent feed", () => {
  ...
  assert.equal(
    shouldForceFeedReconnect({ ...base, lastMessageAt: 80_000 }),
    false,
    "20s silence < 30s threshold -> stay",
  );
  assert.equal(
    shouldForceFeedReconnect({ ...base, lastMessageAt: 70_000 }),
    false,
    "exactly at threshold (not strictly greater) -> stay",
  );
  assert.equal(shouldForceFeedReconnect({ ...base, feedSilenceMs: 0 }), false, "watchdog disabled -> never reconnect");
  assert.equal(
    shouldForceFeedReconnect({ ...base, socketOpen: false }),
    false,
    "closed socket handled by close-driven reconnect, not this",
  );
  assert.equal(
    shouldForceFeedReconnect({ ...base, desiredSubscriptions: 0 }),
    false,
    "nothing subscribed -> not a silent-feed condition",
  );
});
...
test("PolymarketBookClient forces a reconnect when the open book feed goes silent", async () => {
  ...
  assert.ok(
    first.closeCount >= 1,
    "a silent feed forced the socket closed (which triggers a fresh reconnect+resubscribe)",
  );
  client.close();
});
...
test("PolymarketBookClient keeps a live feed connected and pings it (no false reconnect)", async () => {
  ...
  assert.ok(
    sock.sent.some((m) => m === "PING"),
    "heartbeat still pings a healthy socket",
  );
  client.close();
});
...
const kalshiTestKeyPem = generateKeyPairSync("rsa", { modulusLength: 2048 })
...
function withKalshiTestEnv<T>(fn: () => T): T {
  ...
  try {
    ...
    if (prev.id == null) delete process.env.KALSHI_API_KEY_ID;
    else process.env.KALSHI_API_KEY_ID = prev.id;
    if (prev.pk == null) delete process.env.KALSHI_PRIVATE_KEY;
    else process.env.KALSHI_PRIVATE_KEY = prev.pk;
    if (prev.b64 == null) delete process.env.KALSHI_PRIVATE_KEY_B64;
    else process.env.KALSHI_PRIVATE_KEY_B64 = prev.b64;
  }
}
...
test("KalshiTickerClient forces a reconnect when the open orderbook feed goes silent", async () => {
  const { client, first } = withKalshiTestEnv(() => {
    let clockRef = { v: 0 };
    const sockets: FakeBookSocket[] = [];
    const c = new KalshiTickerClient(
      "wss://x",
      () => {
        const s = new FakeBookSocket();
        sockets.push(s);
  ...
  assert.ok(
    first.closeCount >= 1,
    "a silent Kalshi orderbook feed forced the socket closed (triggers reconnect+resubscribe)",
  );
  client.close();
});
