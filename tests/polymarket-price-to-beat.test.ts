...
test("price-to-beat service captures opening and within-tolerance ticks only", async () => {
  ...
  service.setCaptureWindows(
    [
      ...
      {
        marketSlug: "btc-updown-15m-1800000900",
        conditionId: "condition-b",
        eventStartMs: eventStartMs + 15 * 60_000,
        expiryMs: expiryMs + 15 * 60_000,
      },
      {
        marketSlug: "btc-updown-15m-1800001800",
        conditionId: "condition-c",
        eventStartMs: eventStartMs + 30 * 60_000,
        expiryMs: expiryMs + 30 * 60_000,
      },
    ],
    eventStartMs - 1_000,
  );
  ...
  await service.handleTicksForTest([
    { symbol: "btc/usd", value: 76000, timestampMs: eventStartMs, receivedAtMs: eventStartMs },
  ]);
  assert.equal(store.records.get("btc-updown-15m-1800000000")?.priceToBeat, 76000);

  await service.handleTicksForTest([
    { symbol: "btc/usd", value: 76001, timestampMs: eventStartMs + 6_000, receivedAtMs: eventStartMs + 6_000 },
  ]);
