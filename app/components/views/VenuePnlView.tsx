    ...
    <ViewScroll>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatTile
          ...
          value={fmtUsd(p.combinedRealizedTakeHome, { sign: true })}
          tone={realizedTone}
          sub="net of fees · settled"
        />
      ...
      <GridPanel title="Notes" dot="info" span={12} bodyClassName="flex flex-col gap-1.5 p-3">
        ...
        {p.polymarket.unredeemedCount > 0 ? (
          <p className="font-mono text-[11px] leading-relaxed text-amber">
            {p.polymarket.unredeemedCount} resolved Polymarket position(s) are unredeemed — their P&amp;L is realized
            above but the USDC hasn&apos;t been claimed back to your balance yet.
          </p>
        ) : null}
        <p className="font-mono text-[11px] leading-relaxed text-fg-faint">
          Source: venue settlements/positions (not the bot&apos;s signal estimates). The Trade Ledger remains the
          per-signal strategy view.
        </p>
      </GridPanel>
    </ViewScroll>
  );
...
function LegCard({ leg, title, accent }: { leg: VenuePnlLeg; title: string; accent: "kalshi" | "poly" }) {
  ...
  return (
    <GridPanel
      ...
      bodyClassName="p-0"
      right={
        <Badge variant={leg.connectionStatus === "live" ? "up" : "amber"}>
          {leg.connectionStatus === "live" ? "LIVE" : "RECONNECTING"}
        </Badge>
      }
    >
      <div className="flex flex-col">
        <Row
          k="Realized take-home"
          sub="net of fees"
          v={fmtUsd(leg.realizedTakeHome, { sign: true })}
          tone={tone}
          big
        />
        ...
        {accent === "poly" && leg.unredeemedCount > 0 ? (
          <Row k="Unredeemed (resolved)" v={String(leg.unredeemedCount)} tone="amber" />
        ) : null}
      </div>
    </GridPanel>
  );
